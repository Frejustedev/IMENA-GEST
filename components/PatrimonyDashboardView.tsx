...
export const PatrimonyDashboardView: React.FC<PatrimonyDashboardViewProps> = ({ assets, stockItems, onNavigateToInventory, onNavigateToStock, onAddAsset, onAddStockEntry }) => {
    ...
    const {
        ...
        const totalAssetValue = assets.reduce((sum, asset) => sum + (asset.acquisitionCost || 0) * asset.quantity, 0);
        const totalStockValue = stockItems.reduce((sum, item) => sum + item.currentStock * item.unitPrice, 0);
        const functionalCount = assets.filter(a => a.isFunctional).reduce((sum, a) => sum + a.quantity, 0);
        const nonFunctionalCount = assets.filter(a => !a.isFunctional).reduce((sum, a) => sum + a.quantity, 0);

        const familyCounts = assets.reduce((acc, asset) => {
        ...
        const lowStockItems = stockItems.filter(item => item.currentStock <= 10).sort((a,b) => a.currentStock - b.currentStock);
