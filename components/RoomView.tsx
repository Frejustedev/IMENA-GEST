...
export const RoomView: React.FC<RoomViewProps> = ({ 
  ...
  const seenPatients = allPatients
    ...
    .sort((a,b) => {
        const lastActionA = a.history.filter(h => h.roomId === room.id && !h.statusMessage.startsWith('Entré dans')).pop()?.entryDate || '0';
        const lastActionB = b.history.filter(h => h.roomId === room.id && !h.statusMessage.startsWith('Entré dans')).pop()?.entryDate || '0';
