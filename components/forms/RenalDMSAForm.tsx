...
import { Patient, RenalDMSAScintigraphyData } from '../../types';
import { BeakerIcon } from '../icons/BeakerIcon';

interface RenalDMSAFormProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: RenalDMSAScintigraphyData) => void;
  patient: Patient;
  initialData?: RenalDMSAScintigraphyData;
}

type Tab = "info" | "anamnesis" | "bio" | "tech" | "synthesis";

export const RenalDMSAForm: React.FC<RenalDMSAFormProps> = ({
  isOpen,
  onClose,
  onSubmit,
  patient,
  ...
  const [formData, setFormData] = useState<RenalDMSAScintigraphyData>(initialData || {});
        ...
        return (
          <div className="space-y-4">
            <fieldset className="border p-3 rounded-md">
                ...
                <textarea name="antecedents" value={formData.antecedents || ''} onChange={handleInputChange} className={commonTextareaClass} />
            </fieldset>
            <fieldset className="border p-3 rounded-md">
                <legend className="text-md font-semibold px-1">Indication</legend>
                <div className="grid grid-cols-2 gap-4">
                    <fieldset className="border p-2 rounded-md"><legend>Anomalie congénitale</legend>
                        <label className={commonCheckboxLabelClass}><input type="checkbox" name="indication.congenitalAnomaly.jpuSyndrome" checked={!!formData.indication?.congenitalAnomaly?.jpuSyndrome} onChange={handleInputChange}/>Syndrome JPU</label>
                        <label className={commonCheckboxLabelClass}><input type="checkbox" name="indication.congenitalAnomaly.renalDuplication" checked={!!formData.indication?.congenitalAnomaly?.renalDuplication} onChange={handleInputChange}/>Duplication rénale</label>
                    </fieldset>
                    <fieldset className="border p-2 rounded-md"><legend>Infection ou autres</legend>
                        <label className={commonCheckboxLabelClass}><input type="checkbox" name="indication.infectionOrOther.pna" checked={!!formData.indication?.infectionOrOther?.pna} onChange={handleInputChange}/>PNA</label>
                        <label className={commonCheckboxLabelClass}><input type="checkbox" name="indication.infectionOrOther.urinaryInfection" checked={!!formData.indication?.infectionOrOther?.urinaryInfection} onChange={handleInputChange}/>Infection urinaire</label>
                    </fieldset>
                    ...
                    <fieldset className="border p-2 rounded-md"><legend>Lésion morphologique</legend>
                        <label className={commonCheckboxLabelClass}><input type="checkbox" name="indication.morphologicalLesion.abscess" checked={!!formData.indication?.morphologicalLesion?.abscess} onChange={handleInputChange}/>Abcès</label>
                        <label className={commonCheckboxLabelClass}><input type="checkbox" name="indication.morphologicalLesion.cyst" checked={!!formData.indication?.morphologicalLesion?.cyst} onChange={handleInputChange}/>Kyste</label>
                    </fieldset>
                </div>
            </fieldset>
          </div>
            ...
            <div className="space-y-4">
                <fieldset className="border p-3 rounded-md">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="col-span-2"><label className={commonLabelClass}>Technicien</label><input type="text" name="injectionDetails.technician" value={formData.injectionDetails?.technician || ''} onChange={handleInputChange} className={commonInputClass} /></div>
                    </div>
                </fieldset>
                 <fieldset className="border p-3 rounded-md">
                    ...
                    <div className="mt-4"><label className={commonLabelClass}>Vues Statiques (FA/FP/OAD...)</label><input type="text" name="acquisitions.staticViews" value={formData.acquisitions?.staticViews || ''} onChange={handleInputChange} className={commonInputClass} /></div>
                    <div className="mt-4"><label className={commonLabelClass}>Délai Injection-Acquisition</label>
                        <select name="acquisitions.delayInjectionAcquisition" value={formData.acquisitions?.delayInjectionAcquisition || ''} onChange={handleInputChange} className={commonInputClass}>
                            <option value="">Choisir...</option><option value="3H">3H</option><option value="4H">4H</option><option value="5H">5H</option><option value="6H">6H</option><option value="sup6H">sup 6H</option>
                        </select>
                    </div>
                    <div className="mt-4 grid grid-cols-2 gap-4">
                        <label className={commonCheckboxLabelClass}><input type="checkbox" name="acquisitions.spect" checked={!!formData.acquisitions?.spect} onChange={handleInputChange} /><span>SPECT</span></label>
                        <label className={commonCheckboxLabelClass}><input type="checkbox" name="acquisitions.lateAcquisition24H" checked={!!formData.acquisitions?.lateAcquisition24H} onChange={handleInputChange} /><span>Acquisition tardive 24H</span></label>
                    </div>
                </fieldset>
            </div>
        );
        ...
        return (
            <div className="space-y-4">
                <fieldset className="border p-3 rounded-md">
                    ...
                    <div className="mt-2"><label className={commonLabelClass}>Autre</label><textarea name="treatment.other" value={formData.treatment?.other || ''} onChange={handleInputChange} className={commonTextareaClass}/></div>
                </fieldset>
                <fieldset className="border p-3 rounded-md">
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center p-4 z-50">
      <div className="bg-slate-50 rounded-lg shadow-xl w-full max-w-4xl max-h-[95vh] flex flex-col">
        <div className="flex justify-between items-center border-b p-4 bg-white rounded-t-lg">
          <h3 className="text-xl font-semibold text-gray-800 flex items-center space-x-2"><BeakerIcon className="h-6 w-6 text-sky-600"/><span>Fiche Scintigraphie Rénale DMSA: {patient.name}</span></h3>
